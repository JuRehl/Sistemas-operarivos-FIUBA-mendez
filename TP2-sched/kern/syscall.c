...
static int
sys_obtener_prioridad(envid_t envid)
{
	struct Env *dstenv;
	if ((envid2env(envid, &dstenv, 0) == 0))
		return dstenv->env_priority;
	return E_BAD_ENV;
}

static int
sys_modificar_prioridad(void)
{
	curenv->env_priority++;
	return 0;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_obtener_prioridad:
		return sys_obtener_prioridad(a1);
	case SYS_modificar_prioridad:
		return sys_modificar_prioridad();
	case SYS_yield:
		sys_yield();  // No return
	default:
		return -E_INVAL;
